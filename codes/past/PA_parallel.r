...
full_table= foreach(i=seq_along(a),.combine=rbind,
                    .packages = c("tidyverse","tidygraph",
                                  "ggraph","igraph",
                                  "PAFit","widyr")) %dopar% get_cor(a[[i]]) 
...
system.time({parSapply(cl,a,get_cor)})
parSapply(cl,a,get_cor)
stopCluster(cl)
